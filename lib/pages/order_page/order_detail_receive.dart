...
class OrderDetailReceive extends StatefulWidget {
  final int shipperOrderId;
  const OrderDetailReceive({Key? key, required this.shipperOrderId})
      : super(key: key);
  @override
...
class _OrderDetailReceiveState extends State<OrderDetailReceive> {
  ...
  void _changeStatus() {
    int shipperOrderId = item!.shipperOrderId!;
    List<int> listorderid = [];

    for (OrderDetails i in item!.orderDetails!) {
      ProductDetail? productDetailLoad;
      ComboDetail? comboDetailLoad;
      bool key;
      if (i.productDetail != null) {
        productDetailLoad = i.productDetail!;
        key = true;
      } else {
        comboDetailLoad = i.comboDetail!;
        key = false;
      }
      if(key){
        listorderid.add(productDetailLoad!.orderDetailId!);
      }
      else{
                        ...
                        onTap: () {
                          ...
                          if(Currentstatus == 2){
                            ordercontroller.updatestatus(shipperOrderId, listorderid);
                             ...
                             ordercontroller.finishOrder(shipperOrderId, listorderid);
                          }
  ...
  void loadDataOrder() async {
    loaded = false;
    item = ordercontroller.getbyid(widget.shipperOrderId);
    ProductDetail? productDetailLoad;
    ComboDetail? comboDetailLoad;
    bool key;
    if (item!.orderDetails![0].productDetail != null) {
      productDetailLoad = item!.orderDetails![0].productDetail!;
      key = true;
    } else {
      comboDetailLoad = item!.orderDetails![0].comboDetail!;
      key = false;
    }

    if (key) {
      if (productDetailLoad!.status == "Đơn hàng mới")
      {
        liststatus = [true, false, false, false];
        Currentstatus = 1;
      }
        
      else if (productDetailLoad.status == "Đơn hàng đã được xác nhận" ||
          productDetailLoad.status == "Đơn hàng đã chọn được người giao")
      {
        liststatus = [true, true, false, false];
        Currentstatus = 2;
      }
      else if (productDetailLoad.status == "Đơn hàng đang giao")
      {
        Currentstatus = 3;
        liststatus = [true, true, true, false];
      }
      else if (productDetailLoad.status == "Đơn hàng đã hoàn thành")
      {
        liststatus = [true, true, true, true];
        Currentstatus = 4;
      }
    } else {
      if (comboDetailLoad!.status == "Đơn hàng mới")
       {
        liststatus = [true, false, false, false];
        Currentstatus = 1;
      }
      else if (comboDetailLoad.status == "Đơn hàng đã xác nhận" ||
          comboDetailLoad.status == "Đơn hàng đã chọn được người giao")
       {
        liststatus = [true, true, false, false];
        Currentstatus = 2;
      }
        
      else if (comboDetailLoad.status == "Đơn hàng đang giao")
      {
        Currentstatus = 3;
        liststatus = [true, true, true, false];
      }
      else if (comboDetailLoad.status == "Đơn hàng đã hoàn thành")
      {
        liststatus = [true, true, true, true];
        Currentstatus = 4;
      }
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                GestureDetector(
                                  onTap: () {
                                    _ShowSendMessage("${customer!.email}",
                                        "${item!.orderCode}");
                                  },
                                  child: Container(
                                          ...
                                          children: [
                                            ...
                                            Row(
                                              ...
                                              children: [
                                                ...
                                                Text(
                                                    "đ${_formatNumber(productDetail.unitPrice!.toInt())}"),
                                              ],
                                            ),
                                            SizedBox(
                                              height: AppDimention.size10,
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text("Tổng đơn hàng : đ${_formatNumber(item!.totalAmount!.toInt())}"),
                          ],
                        ),
                      ),
                      SizedBox(
                    ...
                    child: Row(
                      ...
                      children: [
                        Container(
                          ...
                          child: Center(
                            child: Text(
                              "đ${_formatNumber(item!.totalAmount!.toInt())}",
                              style: TextStyle(
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.white),
